...
export default {
  ...
  data() {
    ...
    if (
      ...
      this.columnStorage &&
      this.$utils &&
      this.$utils.getStorage &&
      typeof window !== "undefined"
    ) {
      const cacheData = this.$utils.getStorage(this.$route.path);
      if (cacheData.columns) {
        this.columns.forEach((item, i) => {
          const result = cacheData.columns.find(
            (cItem) => cItem.dataIndex === item.dataIndex
          );
  ...
  watch: {
    ...
    loadData: {
      handler(newValue) {
        ...
        if (isObject(newValue) && newValue.list) {
          this.localDataSource = newValue.list;
        }
        if (isArray(newValue) && !newValue.list) {
    ...
    newProps() {
      const attrs = {
        props: {
          ...
          columns: this.newcolumns.filter((i) => i.show),
        },
      ...
      if (this.rowSelection) {
        attrs.props.rowSelection = {
          ...
          onChange: (selectedRowKeys, selectedRows) => {
            ...
            isFunction(this.rowSelection.onChange) &&
  ...
  created() {
    ...
    this.newcolumns = this.columns.map((element) => {
      return {
        show: true,
        ...element,
      };
  ...
  methods: {
    ...
    tableChange(pagination, filters, sorter) {
      ...
      if (isObject(this.loadData)) {
        ...
        this.queryData({ list: this.loadData }, pagination);
        ...
        if (isObject(result)) {
          ...
          result.then((res) => {
            // 处理返回异常处理
            if (!res) {
              res = { list: [], currentPage: 1, total: 1 };
            }
    ...
    /**
     * 格式化表格数据
     * @param {Object} data 列表数据，结构为 { data: { list: [], currentPage: 1, total: 1 } }
    ...
    queryData(data, pagination) {
      ...
      let list = data.list || data.records || data.rows;

      // 如果没数据就初始化一份默认结构 data 可能是请求错误没有list 也做兼容
      if (
        !data ||
      ...
      if (data.total > 0 && pagination?.pageSize > 10) {
        this.scrolltable = { ...{ y: 460 }, ...this.scroll };
      } else {
        this.scrolltable = this.scroll;
      }
      list = data.list || data.records || data.rows;

      // 组装分页参数
      if (this.showPagination && list.length) {
        this.localPagination = {
          ...this.localPagination,
          // 返回结果中的当前分页数
    ...
    dragDataMove(list = [], endIndex) {
      list.forEach((item) => {
        if (item[this.rowKey] === sourceItem[this.rowKey]) {
          isDrop = list.some(
            (children) => children[this.rowKey] === targetItem[this.rowKey]
          );
          // 如果是有效拖动，进行页面数据排序
          if (isDrop) {
            const index = list.findIndex(
              (item) => sourceItem[this.rowKey] === item[this.rowKey]
            );
            // 删除拖拽行之前的位置
            list.splice(index, 1);
    ...
    dragTable() {
      if (this.drag) {
        ...
        const resizeHeader = (h, props, children) => {
          ...
          const col =
            this.newcolumns.find(
              (item) => (item.dataIndex || item.key) === key
            ) || {};
          if (col && !col.width) {
    ...
    columnsCheck(checked, data, index) {
      ...
      this.newcolumns[index].show = checked;
    },
    dragstart1(item) {
      this.oldData = item;
    },
    ...
    dragend1(item, e) {
      if (this.oldData !== this.newData) {
        let oldIndex = this.newcolumns.indexOf(this.oldData);
        let newIndex = this.newcolumns.indexOf(this.newData);
        let newItems = [...this.newcolumns];
        // 删除老的节点
        newItems.splice(oldIndex, 1);
        // 在列表中目标位置增加新的节点
        newItems.splice(newIndex, 0, this.oldData);
        this.newcolumns = [...newItems];
      }
    },
    ...
    renderButton() {
      return (
        <div class="filterClome">
          <div>
            <a-space>{this.$slots.tableButton}</a-space>
          </div>
          {this.columnSort ? this.renderFilter() : null}
        </div>
      );
    },
    renderFilter() {
      return (
        <a-dropdown
          v-model={this.DropdownVisible}
          trigger={["click"]}
          getPopupContainer={(triggerNode) => triggerNode.parentNode}
        >
          <a-menu slot="overlay">
            {this.newcolumns.map((item, index) => (
              <a-menu-item
                key={index}
                draggable={true}
                onDragstart={() => {
                  this.dragstart1(item);
                }}
                onDragenter={(e) => {
                  this.dragenter1(item, e);
                }}
                onDragend={(e) => {
                  this.dragend1(item, e);
                }}
                onDragover={(e) => {
                  this.dragover1(e);
                }}
              >
                <a-checkbox
                  onChange={(e) => {
                    this.columnsCheck(e.target.checked, this.newcolumns, index);
                  }}
                  checked={item.show}
                >
                  {item.title}
    ...
    renderAlert() {
      ...
      return (
        <a-alert showIcon={true} style="margin-bottom: 20px">
  ...
  render() {
    return (
      <div class="h-table-wrapper">
        {this.$slots.tableButton || this.columnSort
          ? this.renderButton()
          : null}
        ...
        <a-table
          class={[
            "h-table",
            this.fixHeight ? "h-table-height1" : "",
            this.fixHeight && this.localDataSource.length > 0
              ? "h-table-height"
              : "",
          ]}
          {...this.newProps}
